...
def draw_love_text(canvas, text, x, y, font, index=0):
    # 控制书写过程
    if index < len(text):
        # 每个字符的位置是逐步递增的
        dx = 0
        dy = 0
        canvas.create_text(x + index * 30 + dx, y + dy, text=text[index], font=font, fill="white")
        canvas.after(200, draw_love_text, canvas, text, x, y, font, index + 1)  # 每200毫秒绘制一个字符
    else:
        # 所有字符完成书写后，调整为等间距展示
        align_text(canvas, text, x, y, font)


def align_text(canvas, text, x, y, font):
    # 书写完后，将字符设置为等间距
    for i in range(len(text)):
        dx = 0  # 不再需要左右摆动
        dy = 0  # 保持字符垂直位置不变
        canvas.create_text(x + i * 30 + dx, y + dy, text=text[i], font=font, fill="white", tags="aligned_text")
    # 每50毫秒更新一次动画
    canvas.after(50, align_text, canvas, text, x, y, font)
...
if __name__ == "__main__":
    ...
    canvas = tk.Canvas(root, width=500, height=500, bg='black')  # 创建画布
    ...
    # 设置意大利斜体字体
    font = ('Times New Roman', 32, 'italic')

    # 启动手写效果
    draw_love_text(canvas, "❤I love you❤", 20, 450, font)
